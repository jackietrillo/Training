...
/*
Chapter 10 bullet list question 13
function modifyMenu(){
	give the form references to form elements in order to validate them.
...
function validateForm(e) {
	...
	if (/^[A-Z \.\-']{2,20}$/i.test(lastName.value)) {
		...
		addErrorMessage('lastName', 'Please enter your first name.');
		error = true;
	}
	
	// Validate city:
	if (/^([A-Za-z])+(\s)*([A-Za-z])+/i.test(city.value)) {
